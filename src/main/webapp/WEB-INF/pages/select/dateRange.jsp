...
<body>
	<div class="container-fluid">
		<div class="od-header">
			...
			<a type="button" id="btn" class="btn btn-sm btn-success" href="downloadMore">
			 <span class="glyphicon glyphicon-download-alt"></span>导出</a> 
		</div>
		<div>	
			开始日期：<input type="text" id="control_date1" class="jeinput" placeholder="YYYY-MM-DD" style="height:21px;width:173px"/>
			截至日期：<input type="text" id="control_date2" class="jeinput" placeholder="YYYY-MM-DD" style="height:21px;width:173px"/>
			 
			 <span class="hidden FileName">${date1}到${date2}(报表)</span>
		</div>
			...
			<table class="table table-responsive table-bordered table-condensed" id="tableExcel">
				<thead>
					 <tr class="warning">
						...
						<th rowspan="2" class="text-center" style="vertical-align: middle !important;width:2%">D%</th>
						<th rowspan="2" class="text-center" style="vertical-align: middle !important;width:2%">CV%</th>
				...
				<tbody>
					<c:forEach items="${nlList}" var="nl">
						<tr class="success text-center operate hidden">
							<td class="noExl hidden">${nlList.indexOf(nl)+1}</td>
							<td>${fn:substring(nl.productionDate,0, 16)}</td>
				...
				<h4 class="text-danger">该范围内没有数据!</h4>
				</c:if>
			</table>
...
$("#control_date1").jeDate({
  format: "YYYY-MM-DD"
});	
$("#control_date2").jeDate({
	  format: "YYYY-MM-DD"
	});
	
$(function() {
	$('.search').click(function() {
		var date1 = $('#control_date1').val();
		var date2 = $('#control_date2').val();
		if(date1!=""&&date2!="") {
			//$(".FileName").text(date1+'-'+date2);
			$(this).attr("href","selectDataRange?date1="+date1+"&date2="+date2).attr("target", "_blank");
		}else {
			alert("请选择日期!");
		}
	});
