	...
	<div class="container-fluid">
		<div class="row">
			...
			<div class="col-sm-11 text-primary">
				<h3 style="text-align: center;margin:0 auto"><b>测量卷曲收缩率和卷曲稳定度</b></h3>
			</div>
		</div>
	
		<div class="pc_header">
			<button class="btn btn-sm btn-primary" data-target="#ajax"
				data-toggle="modal">
				<span class="glyphicon glyphicon-upload"></span> 导入
			</button>
			<a type="button" class="btn btn-sm btn-primary" id="readData">
				...
				<div class="content bm">
					...
					<div class="modal-body text-center">
						<h4 class='<c:choose>
							...
							<c:choose>
								<c:when test="${msg==1}">
								<span class="glyphicon glyphicon-remove danger"></span>上传的文件格式不对</c:when>
								<c:when test="${msg==2}">
				...
				<tbody>
					<c:forEach var="i" begin="${begin}" end="${end}">
						<c:if test="${pages!=null}">
							<tr class="info text-center operate">
								...
								<td>
									...
									<c:choose>
										<c:when test="${pcList.get(i).pcStatus>30 && pcList.get(i).pcStatus<40 }"><p class="text-warning" style="margin: 0"><b>车&nbsp;间&nbsp;加&nbsp;测</b></p></c:when>
									</c:choose>
									<c:choose>
										<c:when test="${pcList.get(i).pcStatus>40 && pcList.get(i).pcStatus<50 }"><p class="text-danger" style="margin: 0"><b>异&nbsp;常&nbsp;丝</b></p></c:when>
									</c:choose>
								</td>
		...
		<div class="cb_footer row">
			...
			<div class="col-md-6 text-center">
		        ...
		        <c:if test="${pages!=null}">
					<li><a href="/pcRead?pages=${pages-1}" title="上一页">&laquo;</a></li>
				</c:if>
					<c:choose>
						<c:when test="${page.getTotalPage()>10}">
							<c:if test="${pages>5&&pages<=page.getTotalPage()-5}">
								...
								<c:if test="${i==pages }">
								<li class="page active"><a href="/pcRead?pages=${i}">${i}</a></li>
								</c:if>
								<c:if test="${i!=pages }">
									<li class="page"><a href="/pcRead?pages=${i}">${i}</a></li>
									</c:if>
								</c:forEach>
							</c:if>
							<c:if test="${pages<=5}">
					...
					<c:if test="${pages!=null}">
					<li class="pages"><a href="/pcRead?pages=${pages+1}" title="下一页">&raquo;</a></li>
					</c:if>
				</ul>
			</div>
			<div class="col-md-3">
...
$("#fileUploadContent").initUpload({
	"uploadUrl": "pc/pcUpFile",//上传文件信息地址
	"progressUrl": "pc/getPercent",//获取进度信息地址，可选，注意需要返回的data格式如下（{bytesRead: 102516060, contentLength: 102516060, items: 1, percent: 100, startTime: 1489223136317, useTime: 2767}）
	//"showSummerProgress":false,//总进度条，默认限制
	//"size":350,//文件大小限制，单位kb,默认不限制
	//"maxFileNumber":15,//文件个数限制，为整数
	"filelSavePath":"E:/project/pc/",//文件上传地址，后台设置的根目录
