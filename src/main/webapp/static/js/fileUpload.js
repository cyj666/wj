    ...
    "addFileList":function(fileList,opt){
        ...
        for(var i=0;i<fileList.length;i++){
            ...
            if(uploadTools.fileIsExit(fileList[i])){
            	/*$('.hide_box p').text("文件（"+fileList[i].name+"）已经存在！");
            	easyDialog.open({
					container : 'testBox'
					//isOverlay : false
				});*/
                layer.alert('文件('+fileList[i].name+')已经存在！', {
					icon: 7,
					skin: 'layui-layer-lan',//样式类名
					closeBtn: 1
					}
            ...
            if(fileSizeLimit!=-1&&fileList[i].size>(fileSizeLimit*1000)){
            	layer.alert('文件('+fileList[i].name+')超出了大小限制！请控制在'+fileSizeLimit+'KB内', {
					icon: 7,
					skin: 'layui-layer-lan',//样式类名
					closeBtn: 1
					}
            ...
            if(fileTypeArray=="*"||uploadTools.isInArray(fileTypeStr,fileTypeArray)){
            	...
            	layer.alert('不支持该格式文件上传,请上传文件后缀为.'+opt.fileType, {
					icon: 7,
					skin: 'layui-layer-lan',//样式类名
					closeBtn: 1
					}
    ...
    "getFileUploadPregressMsg":function(opt){
        ...
        if(progressUrl!="#" && progressUrl!="") {
            var intervalId = setInterval(function(){
                $.get(progressUrl,{},function(dataMap,status){
                    //console.log(dataMap);
                    var percent = dataMap.percent;
                    var bytesRead = dataMap.bytesRead;
                    if(percent >= 100){
